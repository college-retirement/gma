angular.module('gmaApp').controller('AdminProfileListCtrl', function($scope, $location, $http, Persona){
	Persona.status();

	$http.get('/admin/profile').then(function(obj){
	...
	$scope.newPage = function(page) {
		...
		$http.get('/admin/profile?page=' + page + '&sort=' + sort.join(',')).then(function(obj){
			if (!$scope.$$phase) {
