...
class Stronghold {
	...
	private function iterator ($method, $callback) {
		if (array_walk_recursive($this->items, array("Stronghold", $method))) {
			return $callback();
		}
	}
	 ...
	 * @return void
	 */
	private function encrypt(&$value, $index) {
	 ...
	 * @return void
	 */
	private function decrypt(&$value, $index) {
		$value = Crypt::decrypt($value);
	}

	private function isEncrypted(&$value, $index) {
		if (is_string($value)) {
			if (strlen($value) > 20) {
				$this->encrypted = true;
			}
