...
if ($action=="A")
{
	...
	echo createMySchoolsList($schoolIds);
...
else if ($action=="MC")
{
	...
	if ($result)
	{
		...
		echo $todaysDate . "<br><input type=\"button\" class=\"crsButtonSmall\" name=\"btnClear" . $schoolId . "^" . $alertId . "\" id=\"btnClear" . $schoolId . "^" . $alertId . "\" value=\"Clear\" onclick=\"btnClear_onclick(this);\">";
	}
	else
	{
		echo "error in dealineAlertUpdate.php: "  . mysql_error() . "<br><br>SQL:<br>" . $sql;
...
else if ($action=="UC")
{
	...
	if ($result)
	{
		include("../../commonPhp/mySqlClose.php");
		//header("location:deadlineAlert.php");
		echo "<input type=\"button\" class=\"crsButtonSmall\" name=\"btnComplete" . $schoolId . "^" . $alertId . "\" id=\"btnComplete" . $schoolId . "^" . $alertId . "\" value=\"Complete\" onclick=\"btnComplete_onclick(this);\">";
	}
	else
	{
		echo "error in dealineAlertUpdate.php: "  . mysql_error() . "<br><br>SQL:<br>" . $sql;
...
{
	$sortBy=$_POST["sortBy"];
	$sql="SELECT D.COLLEGE_ID, COLLEGE_NAME, B.ALERT_ID, ALERT_DESCRIPTION, DATE_DUE, DATE_COMPLETED, WEB_SITE_URL FROM TDEADLINEALERT_COLLEGES A, TDEADLINEALERT_TYPES B, TUSER_COLLEGE_DEADLINES C, TCOLLEGE D WHERE USER_ID=$userId AND C.COLLEGE_ID IN ($collegeIds) AND C.COLLEGE_ID=A.COLLEGE_ID AND A.COLLEGE_ID=D.COLLEGE_ID AND C.ALERT_ID=B.ALERT_ID AND B.ALERT_ID=A.ALERT_ID ORDER BY ";
	$deadlineList="";

	switch ($sortBy)
	{
		case "name":
			$sql.="COLLEGE_NAME, B.ALERT_ID+0";
			break;
			
		case "deadline";
			$sql.="ALERT_DESCRIPTION, COLLEGE_NAME";
			break;

		case "duedate";
			$sql.="DATE_DUE, COLLEGE_NAME";
			break;

		case "datecompleted";
			$sql.="DATE_COMPLETED, COLLEGE_NAME";
			break;
	}

	echo createDeadlineTable($sql);
}
*/
?>
