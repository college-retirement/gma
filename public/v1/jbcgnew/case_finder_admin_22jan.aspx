   ...
   for(i=0;i<elm.length;i++)
	...
	public void Page_Load() {
        ...
        if (!Page.IsPostBack)
        {
            ...
            string sql2 = "SELECT d.[id] AS [ID], madison_sysdba.users.name AS [Assigned To], d.col0004 + ' ' + d.col0002 AS [Student Name], d.col0018 AS [High School], e.col0002 + ' ' + e.col0003 AS [Parents Name],e.col0018 AS [Email],d.status, cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS [Status] FROM madison_sysdba.mfac_client_data2 AS d INNER JOIN madison_sysdba.users ON d.userid=madison_sysdba.users.userid INNER JOIN madison_sysdba.mfac_parent_info as e ON e.claimid=madison_sysdba.users.userid WHERE  d.userid > 0 and e.col0003 is not null  ORDER BY d.transactionDate DESC";
            string strconn = ConfigurationManager.ConnectionStrings["madison_collegeadvisorsnetworkConnectionString2"].ConnectionString.ToString();
			...
			if(Session["sql"] == null){
			 Session["sql"] = "SELECT d.[id] AS [ID], madison_sysdba.users.name AS [Assigned To], d.col0004 + ',' + d.col0002 AS [Student Name], d.col0018 AS [High School], e.col0002 AS [Parent Last Name], e.col0003 AS [Parents First Name], e.col0018 AS [Email], d.status + ' ' + cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS [Status] FROM " + Session["mainDataTable"] + " AS d INNER JOIN madison_sysdba.users ON d.userid=madison_sysdba.users.userid INNER JOIN madison_sysdba.mfac_parent_info as e ON e.claimid=madison_sysdba.users.userid WHERE";
        //		Session["sql"] += " [status] = 'Payment Authorized' ";
			...
			//Session["sql"] = "SELECT d.[id] AS [ID], users.name AS [Assigned To], d.col0004 + ',' + d.col0002 AS [Student Name], d.col0018 AS [High School], d.status + ' ' + cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS [Status] FROM " + Session["mainDataTable"] + " AS d INNER JOIN madison_sysdba.users ON d.userid=madison_sysdba.users.userid WHERE";
			//Session["sql"] += " d.userid > 0 ";
			sql22 = "SELECT d.[id] AS [ID], madison_sysdba.users.name AS [Assigned To], d.col0004 + ' ' + d.col0002 AS [Student Name], d.col0018 AS [High School], e.col0002 AS [Parent last Name], e.col0003 AS [Parents first Name],e.col0018 AS [Email],d.status, cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS [Status] FROM madison_sysdba.mfac_client_data2 AS d INNER JOIN madison_sysdba.users ON d.userid=madison_sysdba.users.userid INNER JOIN madison_sysdba.mfac_parent_info as e ON e.claimid=d.id WHERE  d.userid > 0  and e.col0003 is not null ORDER BY d.transactionDate DESC";
			//string strconn = ConfigurationManager.ConnectionStrings["madison_collegeadvisorsnetworkConnectionString2"].ConnectionString.ToString();
			////SqlCommand cmd1 = new SqlCommand(sql, strconn);
			//SqlDataAdapter data1 = new SqlDataAdapter(sql2, strconn);
			//DataSet ds1 = new DataSet();
...
//		Session["sql"] += " [status] = 'Payment Authorized' ";
	...
	private void updateSearchQuery(Object sender, EventArgs e) {
		...
		//rebuildSessionQuery();
        Session["sql"] = "SELECT d.[id] AS [ID], madison_sysdba.users.name AS [Assigned To], d.col0004 + ',' + d.col0002 AS [Student Name], d.col0018 AS [High School], e.col0004 AS [Parent's last name] , e.col0002 AS [Parent's first Name], e.col0018 AS [Email], d.status + ' ' + cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS [Status] FROM " + Session["mainDataTable"] + " AS d left JOIN madison_sysdba.mfac_parent_info as e ON e.claimid=d.id left JOIN madison_sysdba.users  ON d.userid=e.claimid WHERE";
        //////////////////SELECT d.[id] AS [ID], madison_sysdba.users.name AS [Assigned To], d.col0004 + ',' + d.col0002 AS [Student Name], d.col0018 AS [High School], e.col0002 AS [Parent's last name] , e.col0002 AS [Parent's first Name], e.col0018 AS [Email], d.status + ' ' + cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS [Status] FROM " + Session["mainDataTable"] + " AS d INNER JOIN madison_sysdba.users  ON d.userid=madison_sysdba.users.userid INNER JOIN madison_sysdba.mfac_parent_info as e ON e.claimid=madison_sysdba.users.userid WHERE";
...
///		Response.Write (sql);
	...
	private string printStatusList(string id) {
        ...
        private string printUserList(string id)
        {
            ...
            string sql = "SELECT * FROM madison_sysdba.users WHERE account_type = 'Representative' or account_type = 'Admin' ORDER BY name";
...
<tr>
	...
	<td align="center" style="width: 903px">
		...
		<%--	<%int a = GridView1.Rows.Count; %>--%>
			<td align="center" style="height: 23px"><font class="black">Matching Leads</font><asp:Label ID="Label1" runat="server" Font-Bold="True" Font-Names="arial" Font-Size="9pt"></asp:Label> </td>
		<%--	<%Label1.Text = " " + a + " Total"; %>--%>
		</tr>
		<%--Date 29 Nov 2007 Akhil Parashar ---GridView1------%>
            ...
            <asp:TemplateField>
             ...
             <asp:TemplateField>
                    <ItemTemplate>
                        <asp:Button ID="btdelete" runat="server" Text="Delete"  CommandName="Deletestudent" CommandArgument='<%# Eval("id") %>'/>
                    </ItemTemplate>
             </asp:TemplateField> 
                </Columns>
	...
	<% }else if(Session["astate"].ToString() == "reassign") {
		//DataSet tmp = getMssqlData(@"SELECT client_last_name + ',' + client_first_name as [Client Name], home_phone as [Home Phone], users.name as [Assigned To], status + ' ' + cast(month(createdDate) as varchar) + '/' + cast(day(createdDate) as varchar) + '/' + cast(year(createdDate) as varchar) as [Status] FROM madison_sysdba.mfac_client_data INNER JOIN users ON users.userid = mfac_client_data.userid WHERE id = '" + Session["reassign"] + "';");
