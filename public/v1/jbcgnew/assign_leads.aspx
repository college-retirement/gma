   ...
   for(i=0;i<elm.length;i++)
	...
	public void Page_Load() {
		...
		if(!Page.IsPostBack) {
            ...
            if(Session["overrideAL"] == null || Session["overrideAL"].ToString() != "override")
			{   
			    Session["astate"] = "list";
			    Session["reassign"] = "";
			}
            Session.Remove("overrideAL");
...
private void cancelReAssign(Object sender, EventArgs e) {
	...
	private void cancelAssign(Object sender, EventArgs e) {
      ...
      private void DeleteGroup1(Object sender, EventArgs e)
        {
            try
            {
                string sql;
                ...
                foreach (GridViewRow row in GridView1.Rows)
                {
                    ...
                    if (check != null && check.Checked)
                    {
                        int id = Convert.ToInt32(GridView1.DataKeys[row.RowIndex].Value);
                        Session["id"] = Convert.ToInt32(id);
                        sql = "Delete " + Session["mainDataTable"] + " WHERE [id] = " + Session["id"] + "";
                        getMssqlData(sql);
                        //updateLog(Session["id"].ToString(), "Assigned");
                        //GridView1.DataBind();
                    }
		...
		GridView1.DataBind();
            ...
            {
                message.Text = "** There was an error processing delete request. **<br>" + ex.StackTrace;
                return;
            }
        }
      ...
      private void DeleteGroup2 (Object sender, EventArgs e) 
          {
              try
              {
                  string sql;
		  ...
		  GridView2.DataBind();
              ...
              {
                  message.Text = "** There was an error processing DeleteGroup request. **<br>" + ex.StackTrace;
                  return;
              }
	      }
	      
	private void DeleteGroup3(Object sender, EventArgs e)
        {
            try
            {
                string sql;
...
<tr>
    ...
    <input id="chkAll"  onclick="javascript:SelectAllCheckboxes(this);" 
              runat="server" type="checkbox"  /> 
                ...
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:oldcrsdb %>" DeleteCommand="DELETE FROM mfac_client_data2 WHERE (id = @id)" SelectCommand="SELECT id AS 'ID', userid,col0004 +', '+ col0002 AS 'Student Name', col0018 AS 'High School', col0013 AS 'Home Phone', status + ' ' + CAST(MONTH(transactionDate) AS varchar) + '-' + CAST(DAY(transactionDate) AS varchar) + '-' + CAST(YEAR(transactionDate) AS varchar) AS 'Status' FROM mfac_client_data2 AS d WHERE (status = 'New Lead') order by d.id desc">
                    <DeleteParameters>
	...
	&nbsp;&nbsp;<font class="black">Assign Selected To: &nbsp;</font><select name="user1"><%= printUserList() %></select> <asp:Button  ID ="Button1" runat="server" onclick="AssignGroup" text="Assign Selected"/> 
		<font class="black">Delete Seleted:</font><asp:Button ID="Deletegroup" runat="server" Text="Delete Selected" OnClientClick="if (!confirm('Are you sure you want to delete selected?')) return;" UseSubmitBehavior="false" onclick="DeleteGroup1"/><%
     ...
     { %>
            ...
            <asp:TemplateField>
                ...
                <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:oldcrsdb %>" DeleteCommand="DELETE FROM mfac_client_data2 WHERE (id = @id)" SelectCommand="select d.id AS 'ID',d.userid,users.name AS 'Assigned To', d.col0004  +', '+ d.col0002 AS 'Student Name', d.col0018 AS 'High School', d.status + ' ' + cast(month(d.transactionDate) AS varchar) + '-' + cast(day(d.transactionDate) AS varchar) + '-' + cast(year(d.transactionDate) AS varchar) AS 'Status' FROM  mfac_client_data2 AS d INNER JOIN users ON d.userid=users.userid WHERE status = 'Assigned' order by d.id desc">
                    <DeleteParameters>
                        <asp:Parameter Name="id" />
                    </DeleteParameters>
                </asp:SqlDataSource>
	...
	<font class="black">Re-Assign Selected To: &nbsp;</font><select name="user2"><%= printUserList() %></select> <asp:Button ID="Button2" runat="server" onclick="ReAssignGroup" text="Re-Assign Selected" /> 
		<font class="black">Delete Seleted:</font><asp:Button ID="Button4" runat="server" Text="Delete Selected" OnClientClick="if (!confirm('Are you sure you want to delete selected?')) return;" UseSubmitBehavior="false" onclick="DeleteGroup2"/><%  		
...
<table border="0" cellpadding="4" cellspacing="4" align="center" valign="top" width="100%">
     ...
     { %>
            ...
            <asp:TemplateField>
                ...
                <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:madison_collegeadvisorsnetworkConnectionString2 %>" DeleteCommand="DELETE FROM mfac_client_data2 WHERE (id = @id)" SelectCommand="select d.id AS 'ID',d.userid,users.name AS 'Assigned To', d.col0004 +', '+  d.col0002 AS 'Student Name',&#13;&#10; d.nextApptDate AS 'Appointment Date', locations.name AS 'Location' &#13;&#10;FROM mfac_client_data2 AS d INNER JOIN users ON d.userid=users.userid&#13;&#10; LEFT JOIN locations ON d.nextApptLocationid=locations.id WHERE status = 'Scheduled' AND nextApptDate > getdate() AND users.userid != 29 order by d.id desc">
                    <DeleteParameters>
                        <asp:Parameter Name="id" />
                    </DeleteParameters>
                </asp:SqlDataSource>
...
<font class="black">Re-Assign Selected To: &nbsp;</font><select name="user3"><%= printUserList() %></select> <asp:Button ID="Button3" runat="server" onclick="ReAssignSchedGroup" text="Re-Assign Selected" /> 
		<font class="black">Delete Seleted:</font><asp:Button ID="Button5" runat="server" Text="Delete Selected" OnClientClick="if (!confirm('Are you sure you want to delete selected?')) return;" UseSubmitBehavior="false" onclick="DeleteGroup3"/><%  		
